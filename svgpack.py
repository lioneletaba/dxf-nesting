...
def nest(output, files, wbin, hbin, enclosing_rectangle=True):
    # packer = newPacker()
    packer = newPacker(mode=PackingMode.Offline, pack_algo=MaxRectsBaf, rotation=True)
...
def main():
    ...
    nest(os.path.join(OUTPUT_PATH, output_file_name), files, 2800, 2000)
